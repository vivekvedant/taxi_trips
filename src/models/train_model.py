...
def train_model(processed_csv_path = None):
        ...
        mlflow.set_tracking_uri("http://localhost:5000")
        mlflow.set_experiment("Decision_Tree Regression test parameter with multicore")
        with mlflow.start_run(run_name="Decision_Tree Regression  test parameter with multicore") :

                Decision_tree_reg = DecisionTreeRegressor(featuresCol = "unscaled_features",labelCol = "fare_amount",maxBins=27,minInstancesPerNode = 77)
